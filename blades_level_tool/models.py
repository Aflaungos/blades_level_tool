...
@dataclass
class Joint:
    """A connection point on a room (from RoomsData or the room prefab)."""
    uid: str
    type: int
    position: Vec3          # grid-space position (RoomsData `_joints`)
    direction: "tuple[float, float]"   # grid-space direction (x, z)
    forbid_as_random_entrance: bool = False
    game_object_name: str = ""   # e.g. "Joint_01" (when parsed from prefab)


@dataclass
class Room:
    """One entry from RoomsData.asset."""
    uid: str
    name: str
    kit_file_id: Optional[int]
    kit_guid: Optional[str]
    size: Vec3                       # grid units
    offset: Vec3                     # grid units
    work_scene_path: str
    size_category: int = 0
    allow_random_selection: bool = False
    use_as_random_entrance: bool = False
    use_as_random_exit: bool = False
